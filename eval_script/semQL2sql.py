    ...
    while len(components) > 0:
        ...
        if isinstance(c_instance, Root):
            ...
            if c_instance.id_c == 1:
                group_n = eval(pop_front(components))
                assert isinstance(group_n, N)
                for i in range(group_n.id_c + 1):
...
def replace_col_with_original_col(query, col, current_table):
    ...
    # print(query, col, current_table.keys())
    # print(cur_table, cur_col, current_table['table_names'], current_table['col_table'], current_table['schema_content_clean'])
    # print(len(current_table['schema_content_clean']), current_table['schema_content_clean'][6])
    for col_ind, col_name in enumerate(current_table['schema_content_clean']):
        # print(col_ind, col_name, cur_col, col_name==cur_col)
        if col_name == cur_col:
            # assert cur_table in current_table['table_names']
            # print(current_table['table_names'][current_table['col_table'][col_ind]], cur_table)
...
def to_str(sql_json, N_T, schema, pre_table_names=None, slot_filling='1'):
    ...
    if 'sup' in sql_json:
        ...
        sup_clause = ('ORDER BY %s %s LIMIT %s' % (subject, direction_map[direction], slot_filling)).strip()
        ...
        for f in sql_json['where']:
            if isinstance(f, str):
                ...
                if value is None:
                    ...
                    filters.append(
                        '%s %s %s' % (subject, op, '(' + to_str(value, len(table_names) + 1, schema) + ')'))
    ...
    if len(sql_json['group']) > 0:
        group_by_clause = "GROUP BY"
        for (agg, col, tab) in sql_json['group']:
    ...
    #                     # if nonfrom_sel == len(sql_json['select']):
    #                     current_table = schema
    ...
    for tab in sql_json['from']:
        col_to_str('none', 'none', tab, table_names, N_T)
...
if __name__ == '__main__':
    ...
    count = 0
    exception_count = 0
    with open(os.path.join(args.output_path, 'pred.txt'), 'w', encoding='utf8') as d, open(os.path.join(args.output_path, 'gold.txt'), 'w', encoding='utf8') as g:
        for i in index:
            try:
                result = transform(datas[i], schemas[datas[i]['db_id']])
                # d_msg = result[0]
                # g_msg = (datas[i]['query'], datas[i]['database_id'])
                d.write(result[0] + '\n')
                # g.write("%s\t%s\t%s\n" % (datas[i]['query'], datas[i]["database_id"], datas[i]["utterance"]))
                g.write("%s\t%s\n" % (datas[i]['query'].replace('\n', ''), datas[i]['db_id']))
                count += 1
            except Exception as e:
                print(traceback.format_exc())
                result = transform(datas[i], schemas[datas[i]['database_id']],
