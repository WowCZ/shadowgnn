...
from src.models.grappa_shadowgnn_rephrase import Shadowgnn
...
def train(args):
    ...
    model = Shadowgnn(args, grammar, vocabulary)
    ...
    try:
        with open(os.path.join(model_save_path, 'epoch.log'), 'w') as epoch_fd:
            for epoch in tqdm.tqdm(range(args.epoch)):
                ...
                model.load_state_dict(torch.load('saved_model/grappa_large_rat_rephrase_generation02020_12_09_12_45_26/best_model.model'))
                schema_item_acc, rephrase_exact_acc, bleu = utils.epoch_rephrase_acc(model, args.batch_size,
                                                                                     val_sql_data,
                                                                                     val_table_data,
                                                                                     beam_size=args.beam_size)
                log_str = 'Epoch: %d, Loss: %f, schema item Acc: %f, rephrase Acc: %f, bleu: %f, time: %f\n' % (
                    epoch + 1, 0, schema_item_acc, rephrase_exact_acc, bleu, 0)

                print(log_str)
                output_file = 'save/predict_rephrase.json'

                utils.dump_rephrase_predict(model, args.batch_size, val_sql_data, val_table_data, output_file, beam_size=3)
                exit(0)

                # tqdm.tqdm.write('fine_tune_alpha: ' + str(fine_tune_alpha.tolist()))
                epoch_begin = time.time()
                loss = utils.epoch_rephrase_train(model, optimizer, scheduler, args.batch_size, sql_data, table_data, args,
                ...
                if schema_item_acc >= best_dev_acc:
                    utils.save_checkpoint(model, os.path.join(model_save_path, 'best_model.model'))
                    best_dev_acc = schema_item_acc

                log_str = 'Epoch: %d, Loss: %f, schema item Acc: %f, rephrase Acc: %f, bleu: %f, time: %f\n' % (
                    epoch + 1, loss, schema_item_acc, rephrase_exact_acc, bleu, epoch_end - epoch_begin)
