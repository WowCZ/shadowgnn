...
def load_dataSets(args):
    ...
    with open(args.table_path, 'r', encoding='utf8') as f:
        table_datas = json.load(f)
    datas = []
    read_range = [re.sub('train', names, args.data_path) for names in
                  ['train_spider_new', 'train_others_new']] if args.data_path == 'data/train.json' else [args.data_path]
    # read_range = [args.data_path]
    for i in read_range:
        print(i)
...
def group_header_all(toks, idx, num_toks, header_toks):
    matched_list = []
    for endIdx in reversed(range(idx + 1, num_toks + 1)):
        sub_toks = toks[idx: endIdx]
        sub_toks = " ".join(sub_toks)
        if sub_toks in header_toks:
            matched_list.append([endIdx, sub_toks])
    return matched_list
...
def check_match(toks, header_toks):
    exact_match = []
    partial_match = []
    header_toks_split = [x.split(' ') for x in header_toks]

    idx = 0
    while idx < len(toks):
        exact_matched = []
        matched_list = group_header_all(toks, idx, len(toks), header_toks)
        for end_idx, matched_header in matched_list:
            for header_idx, header in enumerate(header_toks):
                if header == matched_header:
                    exact_match += [[i, header_idx] for i in range(idx, end_idx)]
                    exact_matched.append(header_idx)

        for header_idx, split_header in enumerate(header_toks_split):
            if header_idx not in exact_matched and toks[idx] in split_header:
                partial_match.append([idx, header_idx])

        idx += 1

    partial_match = [x for x in partial_match if x not in exact_match]
