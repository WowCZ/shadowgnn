...
from src.rule.graph import Graph
from src.rule.semQL import Sup, Sel, Order, Root, Filter, A, N, C, T, Root1, Group, From
from src.rule.sem_utils import alter_inter, alter_not_in, alter_column0, load_dataSets
...
def get_N_T(table_names):
    N_T = len(table_names)
    for x, y in table_names.items():
        assert len(y) == 2 and y[0] == 'T'
        N_T = max(N_T, int(y[1]))
    return N_T
...
def is_end(components, transformed_sql, is_root_processed):
    ...
    if isinstance(c_instance, Root) and is_root_processed:
        ...
        if 'from' not in transformed_sql:
            end = True
        elif len(transformed_sql['from']) == 0:
            end = False
        else:
            end = True
    components.insert(0, c)
    ...
    while len(components) > 0:
        ...
        if isinstance(c_instance, Root):
            ...
            if c_instance.id_c == 0:
                ...
                transformed_sql['where'] = list()
                transformed_sql['sup'] = list()
            elif c_instance.id_c == 8:
                transformed_sql['group'] = list()
                transformed_sql['sup'] = list()
            elif c_instance.id_c == 9:
                transformed_sql['group'] = list()
                transformed_sql['where'] = list()
            elif c_instance.id_c == 10:
                transformed_sql['group'] = list()
                transformed_sql['order'] = list()
        elif isinstance(c_instance, Sel):
            continue
        elif isinstance(c_instance, From):
            table_num = c_instance.id_c + 1
            for _ in range(table_num):
                t = eval(pop_front(components))
                assert isinstance(t, T)
                transformed_sql['from'].append(t.id_c)
        elif isinstance(c_instance, N):
            for i in range(c_instance.id_c + 1):
                agg = eval(pop_front(components))
                column = eval(pop_front(components))
                ...
                transformed_sql['select'].append((
                    agg.production.split()[1],
                    replace_col_with_original_col(col_set[column.id_c], table_names[table.id_c],
                                                  current_table) if table is not None else col_set[column.id_c],
                    table_names[table.id_c] if table is not None else table
                ))
            ...
            # TODO group having
            # transformed_sql['group'].append(c_instance.production.split()[1])
            column = eval(pop_front(components))
            # column = eval(pop_front(components))
            # _table = pop_front(components)
            # table = eval(_table)
            ...
            #     components.insert(0, _table)
            # assert isinstance(agg, A) and isinstance(column, C)
            # transformed_sql['group'].append(agg.production.split()[1])
            # transformed_sql['group'].append(replace_col_with_original_col(col_set[column.id_c], table_names[table.id_c], current_table))
            # transformed_sql['group'].append(table_names[table.id_c] if table is not None else table)
            transformed_sql['group'].append(column.id_c)
            ...
            if op == 'and' or op == 'or':
                ...
                if len(c_instance.production.split()) == 3:
                    if table:
                        fix_col_id = replace_col_with_original_col(col_set[column.id_c], table_names[table.id_c],
                                                                   current_table)
                    else:
                        fix_col_id = col_set[column.id_c]
                        raise RuntimeError('not found table !!!!')
...
def transform(query, schema, origin=None):
    ...
    col_set = query['col_set']
    # table_names = query['table_names']
    # schema['table_names_original'] = query['table_names']
    schema['table_names'] = schema['table_names_original']
...
def infer_from_clause_new(table_names, schema, sql_json):
    table_names_original = schema['table_names_original']
    # table_names_original = schema['table_names']
    join_clause = []
    # print(table_names)
    # print(schema)
    for t_id in sql_json['from']:
        table_name_original = table_names_original[t_id]
        # print(table_name_original)
        if table_name_original not in table_names:
            table_names[table_name_original] = 'T' + str(get_N_T(table_names) + 1)
        alias = table_names[table_name_original]
        join_clause.append((table_name_original, alias))
    # print(table_names)
    # print()
    join_clause_str = ' JOIN '.join(['%s AS %s' % (jc[0], jc[1]) for jc in join_clause])

    on_clause = []
    for jc in join_clause:
        table_id = schema['table_names'].index(jc[0])
        for fk1, fk2 in schema['foreign_keys']:
            if schema['col_table'][fk1] == table_id:
                on_clause.append((fk1, jc[1], schema['column_names_original'][fk1][1]))
            if schema['col_table'][fk2] == table_id:
                on_clause.append((fk2, jc[1], schema['column_names_original'][fk2][1]))
    # print()
    # print(join_clause)
    # print(schema['primary_keys'])
    # print(schema['foreign_keys'])
    # print(schema['col_table'])
    # print(schema['table_names'])
    # print(table_names)
    # print(on_clause)
    # for t_name_1, alias_1 in table_names.items():
    #     t_id_1 = schema['table_names'].index(t_name_1)
    #     for t_name_2, alias_2 in table_names.items():
    #         t_id_2 = schema['table_names'].index(t_name_2)
    #         if t_id_1 == t_id_2:
    #             return 'FROM ' + join_clause_str + ' ON %s.%s = %s.%s' % (alias_1, t_name_1, alias_2, t_name_2)

    for o1 in on_clause:
        for x in schema['foreign_keys']:
            if o1[0] == x[0]:
                for o2 in on_clause:
                    if o2[0] == x[1]:
                        return 'FROM ' + join_clause_str + ' ON %s.%s = %s.%s' % (o2[1], o2[2], o1[1], o1[2])
    return 'FROM ' + join_clause_str
...
def to_str(sql_json, N_T, schema, pre_table_names=None):
    ...
    # if 'from' in sql_json:
    #     for t_id in sql_json['from']:
    #         table_name_original = schema['table_names_original'][t_id]
    #         if table_name_original not in table_names:
    #             table_names[table_name_original] = 'T' + str(len(table_names) + 1)
    # print(table_names)
    ...
    if 'group' in sql_json and len(sql_json['group']) > 0:
        group_by_clause = group_by_clause + ' GROUP BY '
        # print(schema['primary_keys'])
        # print(schema['column_names'])
        # print(schema['table_names'])
        # print(schema['col_set'])
        # print(table_names)
        # TODO: add having
        group_col_id = sql_json['group'][0]
        group_col_real_id = {}
        group_col_name = ''
        # print(schema['col_set'][group_col_id], schema['column_names'])
        for i, (t, c) in enumerate(schema['column_names_original']):
            if schema['col_set'][group_col_id] == schema['column_names'][i][1]:
                group_col_name = c
                group_col_real_id[t] = i
                # break
        # col_original_ids = [i for i, (t_id, c_name) in enumerate(schema['columns']) if c_name == schema['col_set'][group_col_id]]
        if len(sql_json['select']) == 0:
            group_by_clause = group_by_clause + ' ' + group_col_name + ' '
        else:
            flag = False
            for table, table_alias in table_names.items():
                # print(schema['table_names'].index(table))
                # print(group_col_real_id)
                # print([i for i, (t_id, c_name) in enumerate(schema['column_names']) if t_id == schema['table_names'].index(table)])
                table_id = schema['table_names'].index(table)
                if table_id in group_col_real_id:
                    col_real_id = group_col_real_id[table_id]
                    group_by_clause = group_by_clause + ' %s.%s ' % (
                    table_alias, schema['column_names_original'][col_real_id][1])
                    flag = True
                    break
                # if flag:
                #     break
    ...
    #         continue
    #     new_table_names[table_names_replace[key]] = value
    new_table_names = table_names

    # from_clause = infer_from_clause(new_table_names, schema, all_columns).strip()
    from_clause = infer_from_clause_new(new_table_names, schema, sql_json)

    sql = ' '.join(
        [select_clause_str, from_clause, where_clause, group_by_clause, have_clause, sup_clause, order_clause,
         intersect_clause, union_clause, except_clause])
