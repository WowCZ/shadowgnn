...
from src.models.basic_model import BasicModel, HeteroRGCN
...
class IRNet(BasicModel):
    ...
    def sent_see_schema(self, batch, src_encodings, table_embeddings, schema_embeddings):
        ...
        table_col_len = batch.table_col_len
        schema_graphs = batch.reprocess_schema_graphs
        ...
        for bi, (ss_len, t_len, c_len) in enumerate(zip(src_sents_len, table_len, table_col_len)):
            ...
            src_shadow_ctx = torch.bmm(src_schema_att_prob, shadow_item_encoding.unsqueeze(0)).squeeze(0)
        ...
        return src_encoding, table_encoding, column_encoding
    ...
    def forward(self, examples):
        ...
        ori_table_embedding = self.gen_x_batch(batch.table_col_name)
        ori_schema_embedding = self.gen_x_batch(batch.table_names)

        src_encodings, _ = self.encode(batch.src_sents, batch.src_sents_len, None)

        src_encodings, schema_embedding, table_embedding = self.sent_see_schema(batch, src_encodings,
                                                                                  ori_table_embedding,
        ...
        ori_table_embedding = self.gen_x_batch(batch.table_col_name)
        ori_src_embedding = self.gen_x_batch(batch.src_sents)
        ori_schema_embedding = self.gen_x_batch(batch.table_names)
        ...
        col_type = self.input_type(batch.reprocess_col_types)
