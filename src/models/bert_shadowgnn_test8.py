...
class Shadowgnn(BasicModel):
    ...
    def __init__(self, args, grammar):
        ...
        self.tab_att_linear = nn.Linear(args.hidden_size, args.hidden_size)
        self.col_att_linear = nn.Linear(args.hidden_size, args.hidden_size)
        self.src_shadow_cat_linear = nn.Linear(2 * args.hidden_size, args.hidden_size)

        self.shadow_tab_cross_att_v = nn.Linear(args.hidden_size, args.hidden_size)
        self.shadow_col_cross_att_v = nn.Linear(args.hidden_size, args.hidden_size)
        ...
        self.shadow_tab_src_cat_linear = nn.Linear(2 * args.hidden_size, args.hidden_size)
        self.shadow_col_src_cat_linear = nn.Linear(2 * args.hidden_size, args.hidden_size)
    ...
    def sent_see_schema(self, batch, src_encodings, table_embeddings, schema_embeddings, schema_link_drop=False):
        ...
        for bi, (ss_len, t_len, c_len) in enumerate(zip(src_sents_len, table_len, table_col_len)):
            ...
            tab_link_score = schema_link_score[:, :, :t_len]
            col_link_score = schema_link_score[:, :, t_len:]
            ...
            ori_tab_item_encoding = hetero_schema_output['table']
            ori_col_item_encoding = hetero_schema_output['column']
            tab_item_encoding = torch.relu(self.tab_att_linear(ori_tab_item_encoding))
            col_item_encoding = torch.relu(self.col_att_linear(ori_col_item_encoding))

            tab_item_encoding = torch.transpose(tab_item_encoding, 0, 1)
            col_item_encoding = torch.transpose(col_item_encoding, 0, 1)
            ...
            src_tab_att_score = torch.bmm(src_encoding.unsqueeze(0), tab_item_encoding.unsqueeze(0))
            src_col_att_score = torch.bmm(src_encoding.unsqueeze(0), col_item_encoding.unsqueeze(0))

            assert src_tab_att_score.shape == tab_link_score.shape
            src_tab_att_score = src_tab_att_score * tab_link_score
            src_col_att_score = src_col_att_score * col_link_score
            src_schema_att_score = torch.cat((src_tab_att_score, src_col_att_score), dim=2)

            src_tab_att_prob = torch.softmax(src_tab_att_score.squeeze(0), dim=1)
            src_col_att_prob = torch.softmax(src_col_att_score.squeeze(0), dim=1)
            ...
            shadow_tab_item_encoding_v = torch.relu(self.shadow_tab_cross_att_v(shadow_tab_item_encoding))
            shadow_col_item_encoding_v = torch.relu(self.shadow_col_cross_att_v(shadow_col_item_encoding))
            ...
            shadow_src_tab_ctx = shadow_src_ctx[:t_len, :]
            shadow_src_col_ctx = shadow_src_ctx[t_len:, :]

            shadow_src_tab_ctx = torch.cat((shadow_tab_item_encoding, shadow_src_tab_ctx), dim=1)
            shadow_src_tab_ctx = torch.relu(self.shadow_tab_src_cat_linear(shadow_src_tab_ctx))
            shadow_src_col_ctx = torch.cat((shadow_col_item_encoding, shadow_src_col_ctx), dim=1)
            shadow_src_col_ctx = torch.relu(self.shadow_col_src_cat_linear(shadow_src_col_ctx))

            table_shadow_encoding = shadow_src_tab_ctx
            column_shadow_encoding = shadow_src_col_ctx
            ...
            src_total_repre.append(
                torch.cat((src_shadow_ctx_encoding, table_shadow_encoding, column_shadow_encoding), dim=0))
            ...
            src_total_lens.append(ss_len + t_len + c_len)

        src_total_encoding = get_batch_embedding(src_total_repre, src_total_lens, self.args.cuda)
        ...
        return src_total_encoding, src_total_lens, batch_max_src_schema_att_prob
    ...
    def forward(self, examples):
        ...
        src_total_encoding, src_total_lens, batch_max_att_p = self.sent_see_schema(batch, ori_src_embedding,
                                                                                  ori_table_embedding,
                                                                                  ori_schema_embedding)
        src_total_encoding, (last_state, last_cell) = self.encode_again(src_total_encoding, src_total_lens)

        schema_embedding = []
        table_embedding = []
        for bi, (ss_len, t_len, c_len) in enumerate(batch_stc_lens):
            schema_embedding.append(src_total_encoding[bi][ss_len: ss_len + t_len])
            table_embedding.append(src_total_encoding[bi][ss_len + t_len: ss_len + t_len + c_len])
        schema_embedding = get_batch_embedding(schema_embedding, batch.table_len, self.args.cuda)
        table_embedding = get_batch_embedding(table_embedding, batch.col_num, self.args.cuda)

        src_total_encoding = self.dropout(src_total_encoding)

        utterance_encodings_sketch_linear = self.att_sketch_linear(src_total_encoding)
        utterance_encodings_lf_linear = self.att_lf_linear(src_total_encoding)
        ...
        for t in range(batch.max_sketch_num):
            ...
            src_mask = batch.src_total_token_mask

            (h_t, cell_t), att_t, aw = self.step(x, h_tm1, src_total_encoding,
    ...
    def parse(self, examples, beam_size=5):
        ...
        while len(completed_beams) < beam_size and t < self.args.decode_max_time_step:
            hyp_num = len(beams)
            exp_src_enconding = src_total_encoding.expand(hyp_num, src_total_encoding.size(1),
                                                     src_total_encoding.size(2))
        ...
        while len(completed_beams) < beam_size and t < self.args.decode_max_time_step:
            ...
            # expand value
            exp_src_encodings = src_total_encoding.expand(hyp_num, src_total_encoding.size(1),
                                                     src_total_encoding.size(2))
            ...
            for hyp_id, hyp in enumerate(beams):
                ...
                if type(padding_sketch[t+j]) == define_rule.A or type(padding_sketch[t+j]) == define_rule.V or type(padding_sketch[t+j]) == define_rule.C1:
