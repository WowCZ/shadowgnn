...
class Shadowgnn(BasicModel):
    ...
    def __init__(self, args, grammar):
        ...
        etypes = ['norm_t2c', 'norm_c2t', 'prim_t2c', 'prim_c2t', 'fore_c2c', 'fore_invc2c', 's2t', 't2s', 'fore_t2c', 'fore_c2t']
        # 9,19,43
        ...
        self.shadow_norm_layer = []
        self.shadow_fnn_layer = []
        ...
        if args.cuda:
            for l in range(self.layer_num):
                ...
                self.shadow_norm_layer[l] = self.shadow_norm_layer[l].cuda()
                self.shadow_fnn_layer[l] = self.shadow_fnn_layer[l].cuda()
        ...
        input_dim = args.action_embed_size + args.type_embed_size
    ...
    def forward(self, examples):
        ...
        for t in range(batch.max_action_num):
            if t == 0:
                x = torch.zeros((len(batch), self.lf_decoder_lstm.input_size), dtype=torch.float)
                if self.args.cuda:
                    x = x.cuda()
            else:
                a_tm1_embeds = []
        ...
        return [lf_prob_var, att_prob_var]
    ...
    def parse(self, examples, beam_size=5):
        ...
        while len(completed_beams) < beam_size and t < self.args.decode_max_time_step:
            ...
            if t == 0:
                with torch.no_grad():
                    x = torch.zeros((1, self.lf_decoder_lstm.input_size), dtype=torch.float)
                    if self.args.cuda:
                        x = x.cuda()
            else:
                a_tm1_embeds = []
        ...
        if len(completed_beams) > 0:
            for act in completed_beams[0].actions:
                if type(act) in [define_rule.Root1,
                                        define_rule.Root,
                                        define_rule.Sel,
                                        define_rule.Filter,
                                        define_rule.A,
