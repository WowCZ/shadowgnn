...
from src.models.grappa_shadowgnn_multitask import Shadowgnn
    ...
    try:
        with open(os.path.join(model_save_path, 'epoch.log'), 'w') as epoch_fd:
            for epoch in tqdm.tqdm(range(args.epoch)):
                ...
                # json_datas, schema_item_acc, rephrase_exact_acc, bleu = utils.epoch_multitask_acc(model,
                #                                                                                   args.batch_size,
                #                                                                                   val_sql_data,
                #                                                                                   val_table_data,
                ...
                loss = utils.epoch_multitask_train(model, optimizer, scheduler, args.batch_size, sql_data, table_data, args,
                ...
                epoch_end = time.time()
                json_datas, schema_item_acc, rephrase_exact_acc, bleu = utils.epoch_multitask_acc(model, args.batch_size,
                                                                      val_sql_data,
                                                                      val_table_data,
                                                                      beam_size=args.beam_size)
                ...
                log_str = 'Epoch: %d, Loss: %f, schema item Acc: %f, rephrase Acc: %f, bleu: %f, EM ACC: %f, time: %f\n' % (
                    epoch + 1, loss, schema_item_acc, rephrase_exact_acc, bleu, spider_acc[-1], epoch_end - epoch_begin)
